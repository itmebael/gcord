...
(function (global) {
  ...
  async function registerSignup(payload) {
    var sb = requireClient();
    var parts = String(payload.full_name || '').trim().split(/\s+/);
    var first = parts[0] || 'User';
    var last = parts.slice(1).join(' ') || first;
    var passwordHash = await hashPassword(String(payload.password || ''));

    var rpc = await sb.rpc('app_register_staff', {
      p_email: String(payload.email || '').trim(),
      p_password: passwordHash,
      p_first_name: first,
      p_last_name: last,
      p_age: payload.age != null ? Number(payload.age) : null,
      p_birthday: payload.birthday || null,
      p_valid_id_url: payload.valid_id_url || '',
      p_face_capture_url: payload.face_capture_url || '',
      p_phone: payload.phone || null
    });

    if (!rpc.error && rpc.data) {
      var row = Array.isArray(rpc.data) ? rpc.data[0] : rpc.data;
      if (!row || row.ok === false) throw new Error((row && row.message) || 'Signup failed.');
      return row;
    }

    // Fallback if RPC not installed (requires admin RLS open / migration)
    return createUser({
      first_name: first,
      last_name: last,
      email: payload.email,
  ...
  async function findTransactionByRef(ref) {
    ...
    var res = await sb.from('transactions').select('id,ref_no,status,created_by_user_id').eq('ref_no', key).order('id', { ascending: true }).limit(1);
    if (res.error) throw res.error;
  ...
  async function addTransaction(payload) {
    ...
    if (!txnDate && payload.date) {
      ...
      if (!isNaN(parsed.getTime())) txnDate = parsed.toISOString().slice(0, 10);
    }
    if (!txnDate) txnDate = new Date().toISOString().slice(0, 10);
    ...
    return { transaction: mapTransaction(res.data), isDuplicate: status === 'duplicate' };
  }
  ...
  async function listDuplicateEvents() {
      ...
      return txns
        ...
        .map(function (t) {
          return {
            ...
            origStatus: 'Verified',
            origDate: t.date + (t.time && t.time !== '—' ? ' • ' + t.time : ''),
    ...
    if (ids.length) {
      ...
      var txnRes = await sb
        .from('transactions')
        .select('id,ref_no,amount,txn_date,txn_time,status')
        .in('id', uniq);
      if (!txnRes.error) {
    ...
    return rows.map(function (r) {
      ...
      return {
        ...
        origDate: orig ? fmtWhen(orig.txn_date, orig.txn_time) : when,
        ...
        matchDate: dup ? fmtWhen(dup.txn_date, dup.txn_time) : when
      };
    });
  }
